...
const TYPE_COLORS: Record<string, string> = {
  PAYROLL_APPROVED: "bg-green-100 text-green-600",
  PAYROLL_GENERATED: "bg-blue-100 text-blue-600",
...
export default function HRNotificationsPage() {
  ...
  const markAllRead = async () => {
    const res = await fetch("/api/hr/notifications/read-all", { method: "POST" });
    if (res.ok) {
  ...
  const markRead = async (id: string) => {
    await fetch(`/api/hr/notifications/${id}/read`, { method: "POST" });
