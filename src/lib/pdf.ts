...
// Navy + gold theme (ECOMFLEX reference)
const C = {
  navy:    "#1a2744", navyLt:  "#eef1f8", navyMid: "#c8d2e8", gold:    "#c8960a",
  earHd:   "#1a2744", earHdBg: "#e8edf7", earColBg:"#d0d8ec", earA:    "#eef1f8", earB:    "#f8f9fc", earLine: "#c0c8dc",
  dedHd:   "#1a2744", dedHdBg: "#e8edf7", dedColBg:"#d0d8ec", dedA:    "#eef1f8", dedB:    "#f8f9fc", dedLine: "#c0c8dc",
...
export async function generatePayslipPDF(data: PayslipPDFData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    if (fs.existsSync(LOGO_PATH)) {
      ...
      doc.font("Bold").fontSize(22).fillColor(C.gold).text("W", M + 15, HDR_H / 2 - 14);
    }
    ...
    doc.font("Reg").fontSize(9).fillColor("#9badc8")
      .text("#5/BC-106, 5th Cross, HRBR Layout, 3rd Block, Kalyan Nagar, Bangalore - 560043", txX, 36, { width: 220 })
      .text("Ph: 080-41233006  |  hr@wecoolimports.com", txX, 58, { width: 220 });

    // PAY SLIP badge
    const bW = 140, bH = 74, bx = PW - M - bW, by = (HDR_H - bH) / 2 - 2;
    r(doc, bx, by, bW, bH, "#0f1a30");
    r(doc, bx, by,        bW, 4, C.gold);
    r(doc, bx, by + bH - 4, bW, 4, C.gold);
    doc.font("Bold").fontSize(15).fillColor(C.gold)
    ...
    doc.font("Bold").fontSize(10.5).fillColor(C.gold)
    ...
    doc.font("Bold").fontSize(11.5).fillColor(C.gold).text("SALARY DETAILS", M, y + mid(HDG, 11.5), { width: CW, align: "center" });
    y += HDG + GAP;
    ...
    doc.font("Bold").fontSize(24).fillColor(C.gold)
    ...
    // ── LEAVE SUMMARY ────────────────────────────────────────────────────────
    r(doc, M, y, CW, HDG, C.navy, C.navyMid, 0.8);
    doc.font("Bold").fontSize(11.5).fillColor(C.gold)
